...
export const createBooking = async (req, res, next) => {
  try {
    ...
    for (const { name, productId, quantity } of products) {
      ...
      if (!productInventory) {
        return next(
          createError(`Product ${name} not found in inventory.`, 404),
        );
      }
      ...
      if (quantity > available) {
        // check to make sure this is how u get this
        return next(
          createError(
            `Only ${productInventory.available} ${name} available.`,
            400,
          ),
        );
      }
...
export const getBooking = async (req, res, next) => {
  try {
    ...
    if (!booking) {
      return next(
        createError(`Booking with bookingId ${bookingId} not found.`, 404),
      );
    }
...
export const getTodaysBookings = async (req, res, next) => {
  try {
    ...
    const allBookingData = await Promise.all(
      todaysBookings.map(async (booking) => {
        const { name, email } = await Customer.findById(
          booking.customer.customerId,
        );

        if (!name || !email) {
          throw createError(
            `Customer not found for booking ID ${booking._id}`,
            404,
          );
        }
...
export const getUpcomingBookings = async (req, res, next) => {
  try {
    ...
    const upcomingBookings = await Booking.find({
      ...
      status: { $nin: ["cancelled", "complete", "returned"] },
    }).sort({ pickupDate: 1 });
    ...
    const customerIds = upcomingBookings.map(
    ...
    const allBookingData = upcomingBookings.map((booking) => {
      const customer = customerMap[booking.customer.customerId.toString()];
      ...
      const quantity = booking.products.reduce(
...
export const getBookingsMetrics = async (req, res, next) => {
  try {
    ...
    const totalProductQuantity =
      quantityAggregation.length > 0 ? quantityAggregation[0].total : 0;
    ...
    const totalAmountDue =
      dueAggregation.length > 0 ? dueAggregation[0].total : 0;
    ...
    if (updates.status === "returned" || updates.status === "cancelled") {
      ...
      for (const product of products) {
        ...
        await Inventory.findOneAndUpdate(
          ...
          {
            $inc: { available: product.quantity, reserved: -product.quantity },
          }, // Use $inc for safer updates
        );
      }
    }
...
export const updateUserBookingStatus = async (req, res, next) => {
  try {
    ...
    if (!booking) {
      return next(
        createError("Booking not found or you are not authorized.", 404),
      );
    }
